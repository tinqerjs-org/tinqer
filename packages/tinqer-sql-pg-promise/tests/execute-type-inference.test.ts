...
async function typeTests() {
  ...
  const users: {
    ...
    active: boolean;
    deptId: number;
  }[] = await executeSelect(mockDb, db, (q) => q.from("users"), {});
  ...
  const firstUser: {
    ...
    active: boolean;
    deptId: number;
  } = await executeSelect(mockDb, db, (q) => q.from("users").first(), {});
  ...
  const maybeUser:
    ...
    | undefined = await executeSelect(mockDb, db, (q) => q.from("users").firstOrDefault(), {});

  // count() returns number
  const count: number = await executeSelect(mockDb, db, (q) => q.from("users").count(), {});

  // any() returns boolean
  const hasUsers: boolean = await executeSelect(mockDb, db, (q) => q.from("users").any(), {});
