...
describe("Execute Function", () => {
  describe("Basic queries", () => {
    it("should execute a simple query and return typed results", async () => {
      ...
      const sqlResult = selectStatement(db, (q) => q.from("users"), {});
    ...
    it("should execute with WHERE clause", async () => {
      const sqlResult = selectStatement(db, (q) => q.from("users").where((u) => u.age >= 18), {});
  ...
  describe("Terminal operations", () => {
    it("should handle first() operation", async () => {
      const sqlResult = selectStatement(db, (q) => q.from("users").first(), {});
    ...
    it("should handle first() with predicate", async () => {
      const sqlResult = selectStatement(db, (q) => q.from("users").first((u) => u.id === 1), {});
    ...
    it("should handle single() operation", async () => {
      const sqlResult = selectStatement(db, (q) => q.from("users").single((u) => u.id === 1), {});
    ...
    it("should handle any() operation with predicate", async () => {
      const sqlResult = selectStatement(db, (q) => q.from("users").any((u) => u.age >= 18), {});
      expect(sqlResult.sql).to.equal(
    ...
    it("should handle all() operation", async () => {
      const sqlResult = selectStatement(db, (q) => q.from("users").all((u) => u.isActive), {});
      expect(sqlResult.sql).to.equal(
