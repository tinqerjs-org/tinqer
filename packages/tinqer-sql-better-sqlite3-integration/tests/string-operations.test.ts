  ...
  describe("startsWith", () => {
    it("should find users with names starting with 'J'", () => {
      ...
      const results = executeSelectSimple(
        db,
        dbContext,
        (ctx) => ctx.from("users").where((u) => u.name.startsWith("J")),
        {
          onSql: (result) => {
            capturedSql = result;
          },
    ...
    it("should find emails starting with specific prefix", () => {
      ...
      const results = executeSelectSimple(
        db,
        dbContext,
        (ctx) => ctx.from("users").where((u) => u.email.startsWith("alice")),
        {
          onSql: (result) => {
            capturedSql = result;
          },
    ...
    it("should combine startsWith with other conditions", () => {
      ...
      const results = executeSelectSimple(
        db,
        dbContext,
        (ctx) => ctx.from("users").where((u) => u.name.startsWith("J") && u.is_active === 1),
        {
          onSql: (result) => {
            capturedSql = result;
          },
  ...
  describe("endsWith", () => {
    it("should find emails ending with '@example.com'", () => {
      ...
      const results = executeSelectSimple(
        db,
        dbContext,
        (ctx) => ctx.from("users").where((u) => u.email.endsWith("@example.com")),
        {
          onSql: (result) => {
            capturedSql = result;
          },
  ...
  describe("contains (includes)", () => {
    it("should find users with 'oh' in their name", () => {
      ...
      const results = executeSelectSimple(
        db,
        dbContext,
        (ctx) => ctx.from("users").where((u) => u.name.includes("oh")),
        {
          onSql: (result) => {
            capturedSql = result;
          },
  ...
  describe("Complex string queries", () => {
    ...
    it("should handle case-sensitive string operations", () => {
      ...
      const upperResults = executeSelectSimple(
        db,
        dbContext,
        (ctx) => ctx.from("users").where((u) => u.name.includes("J")),
        {
          onSql: (result) => {
            capturedSql1 = result;
          },
      ...
      const lowerResults = executeSelectSimple(
        db,
        dbContext,
        (ctx) => ctx.from("users").where((u) => u.name.includes("o")),
        {
          onSql: (result) => {
            capturedSql2 = result;
          },
      ...
      const capitalD = executeSelectSimple(
        db,
        dbContext,
        (ctx) => ctx.from("users").where((u) => u.name.includes("D")),
        {
          onSql: (result) => {
            capturedSql3 = result;
          },
  ...
  describe("String pattern escaping", () => {
    ...
    it("should handle underscore in search term", () => {
      ...
      const results = executeSelect(
        db,
        dbContext,
        (ctx, params) => ctx.from("test_underscores").where((n) => n.name.includes(params.search)),
        { search: "_" },
        {
          onSql: (result) => {
            capturedSql = result;
    ...
    it("should handle backslash in search term", () => {
      ...
      const results = executeSelect(
        db,
        dbContext,
        (ctx, params) => ctx.from("test_backslashes").where((p) => p.path.includes(params.search)),
        { search: "\\" },
        {
          onSql: (result) => {
            capturedSql = result;
    ...
    it("should handle multiple special characters together", () => {
      ...
      const results = executeSelect(
        ...
        dbContext,
        (ctx, params) =>
          ctx.from("test_mixed_special").where((c) => c.content.includes(params.search)),
        { search: "%_" },
        {
          onSql: (result) => {
            capturedSql = result;
