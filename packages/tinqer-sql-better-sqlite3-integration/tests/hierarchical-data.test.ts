 ...
 * Tests for self-referential and tree-structured data queries with real Better SQLite3.
 */
...
describe("Better SQLite3 Integration - Hierarchical Data", () => {
  ...
  describe("Parent-child relationships", () => {
    ...
    it("should find children of specific parent", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
      ...
      expect(capturedSql).to.exist;
      expect(capturedSql!.sql).to.equal('SELECT * FROM "categories" WHERE "parent_id" = @parentId');
    ...
    it("should find leaf nodes", () => {
      ...
      results.forEach((category) => {
        expect(category.is_leaf).to.equal(1);
        expect(category.level).to.equal(2);
      });
    });

    it("should find nodes at specific level", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
      const targetLevel = 1;
      const results = executeSelect(
        db,
      ...
      expect(capturedSql).to.exist;
      expect(capturedSql!.sql).to.equal('SELECT * FROM "categories" WHERE "level" = @targetLevel');
  ...
  describe("Path-based queries", () => {
    ...
    it("should find descendants using path", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
  ...
  describe("Level-based queries", () => {
    it("should find nodes within level range", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
      const minLevel = 1;
      const maxLevel = 2;
      const results = executeSelect(
  ...
  describe("Manager-employee relationships", () => {
    ...
    it("should find all subordinates of a manager", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
  ...
  describe("Comment thread patterns", () => {
    ...
    it("should find replies to specific comment", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
    ...
    it("should find comments by depth", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
      const maxDepth = 1;
      const results = executeSelect(
        db,
  ...
  describe("Self-join patterns", () => {
    ...
    it("should find siblings (same parent)", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
      const parentId = 1; // Electronics
      const results = executeSelect(
        db,
  ...
  describe("Complex hierarchical queries", () => {
    it("should find categories with specific parent and level", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
      ...
      expect(capturedSql!.params).to.deep.equal({ parentId: 2, level: 2, __p1: 1 });
  ...
  describe("Performance-oriented hierarchical patterns", () => {
    it("should use path for subtree queries", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
    ...
    it("should efficiently check ancestry using path", () => {
      let capturedSql: { sql: string; params: Record<string, unknown> } | undefined;
