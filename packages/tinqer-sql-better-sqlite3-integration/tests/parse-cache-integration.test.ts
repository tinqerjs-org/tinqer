...
describe("Parse Cache Integration Tests (SQLite)", () => {
  ...
  describe("SELECT query caching", () => {
    it("should cache repeated SELECT queries", () => {
      ...
      const result1 = executeSelectSimple(db, schema, (q, _params, _helpers) =>
      ...
      const result2 = executeSelectSimple(db, schema, (q, _params, _helpers) =>
        q.from("users").where((u) => u.age !== null && u.age >= 18),
      );
      expect(parseCache.size()).to.equal(1);
    ...
    it("should bypass cache when cache option is false", () => {
      ...
      executeSelectSimple(db, schema, (q, _params, _helpers) =>
        q.from("users").where((u) => u.age !== null && u.age >= 18),
      );
    ...
    it("should cache terminal operations (count, sum, etc.)", () => {
      executeSelectSimple(db, schema, (q, _params, _helpers) => q.from("users").count());
      executeSelectSimple(db, schema, (q, _params, _helpers) =>
        q
          .from("users")
          .where((u) => u.age !== null)
  ...
  describe("DELETE statement caching", () => {
    it("should cache repeated DELETE statements", () => {
      ...
      executeDelete(db, schema, (q, p) => q.deleteFrom("users").where((u) => u.id === p.userId), {
        userId: userId1.id,
      });
