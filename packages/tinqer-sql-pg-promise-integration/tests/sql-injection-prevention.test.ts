...
describe("PostgreSQL Integration - SQL Injection Prevention", () => {
  ...
  describe("String literals with SQL keywords", () => {
    it("should safely handle DROP TABLE in string literals", async () => {
      ...
      const tableCheck = await executeSelectSimple(db, (ctx) => ctx.from("users").take(1), {
        onSql: (result) => {
    ...
    it("should safely handle UPDATE commands in strings", async () => {
      ...
      const activeUsers = await executeSelectSimple(
        db,
        (ctx) => ctx.from("users").where((u) => u.is_active == true),
        {
          onSql: (result) => {
    ...
    it("should safely handle DELETE commands in strings", async () => {
      ...
      const orders = await executeSelectSimple(db, (ctx) => ctx.from("orders").take(1), {
        onSql: (result) => {
          ordersSql = result;
        },
      });
  ...
  describe("Stacked query attempts", () => {
    it("should prevent semicolon-separated queries", async () => {
      ...
      const userCountBefore = await executeSelectSimple(db, (ctx) => ctx.from("users"));
      ...
      const userCountAfter = await executeSelectSimple(db, (ctx) => ctx.from("users"));
    ...
    it("should handle multiple semicolons", async () => {
      ...
      const products = await executeSelectSimple(db, (ctx) => ctx.from("products").take(1), {
        onSql: (result) => {
          productsSql = result;
        },
      });
  ...
  describe("Complex injection in different contexts", () => {
    it("should handle injection attempts in ORDER BY context", async () => {
      ...
      const users = await executeSelectSimple(db, (ctx) => ctx.from("users").take(1), {
        onSql: (result) => {
          usersSql = result;
        },
      });
  ...
  describe("Binary payload injection attempts", () => {
    ...
    it("should handle control characters in parameters", async () => {
      ...
      const results = await executeSelect(
        db,
        (ctx, params) => ctx.from("products").where((p) => p.name == params.controlCharsPayload),
        { controlCharsPayload },
        {
          onSql: (result) => {
            capturedSql = result;
    ...
    it("should handle SQL injection with special characters", async () => {
      ...
      const results = await executeSelect(
        db,
        (ctx, params) => ctx.from("users").where((u) => u.name == params.mixedPayload),
        { mixedPayload },
        {
          onSql: (result) => {
            capturedSql = result;
  ...
  describe("Complex nested injection attempts", () => {
    it("should handle nested conditions with injection attempts", async () => {
      ...
      const users = await executeSelectSimple(db, (ctx) => ctx.from("users"));
