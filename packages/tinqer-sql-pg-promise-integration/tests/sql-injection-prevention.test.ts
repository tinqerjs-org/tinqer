...
describe("PostgreSQL Integration - SQL Injection Prevention", () => {
  ...
  describe("String literals with SQL keywords", () => {
    it("should safely handle DROP TABLE in string literals", async () => {
      ...
      const tableCheck = await executeSelectSimple(db, () => from(dbContext, "users").take(1), {
        onSql: (result) => {
    ...
    it("should safely handle DELETE commands in strings", async () => {
      ...
      const orders = await executeSelectSimple(db, () => from(dbContext, "orders").take(1), {
        onSql: (result) => {
          ordersSql = result;
        },
      });
  ...
  describe("Stacked query attempts", () => {
    it("should prevent semicolon-separated queries", async () => {
      ...
      const userCountBefore = await executeSelectSimple(db, () => from(dbContext, "users"));
      ...
      const userCountAfter = await executeSelectSimple(db, () => from(dbContext, "users"));
    ...
    it("should handle multiple semicolons", async () => {
      ...
      const products = await executeSelectSimple(db, () => from(dbContext, "products").take(1), {
        onSql: (result) => {
          productsSql = result;
        },
      });
  ...
  describe("Complex injection in different contexts", () => {
    it("should handle injection attempts in ORDER BY context", async () => {
      ...
      const users = await executeSelectSimple(db, () => from(dbContext, "users").take(1), {
        onSql: (result) => {
          usersSql = result;
        },
      });
  ...
  describe("Complex nested injection attempts", () => {
    it("should handle nested conditions with injection attempts", async () => {
      ...
      const users = await executeSelectSimple(db, () => from(dbContext, "users"));
