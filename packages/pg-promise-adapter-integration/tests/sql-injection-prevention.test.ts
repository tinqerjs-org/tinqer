...
import { executeSelect, executeSelectSimple } from "@tinqerjs/pg-promise-adapter";
...
describe("PostgreSQL Integration - SQL Injection Prevention", () => {
  ...
  describe("String literals with SQL keywords", () => {
    ...
    it("should safely handle UPDATE commands in strings", async () => {
      ...
      const activeUsers = await executeSelectSimple(
        dbClient,
        schema,
        (q) => q.from("users").where((u) => u.is_active == true),
        {
          onSql: (result) => {
      ...
      expect(activeUsersSql!.params).to.deep.equal({ __p1: true });
    ...
    it("should safely handle DELETE commands in strings", async () => {
      ...
      const orders = await executeSelectSimple(dbClient, schema, (q) => q.from("orders").take(1), {
        onSql: (result) => {
          ordersSql = result;
        },
      });
  ...
  describe("Stacked query attempts", () => {
    it("should prevent semicolon-separated queries", async () => {
      ...
      const userCountBefore = await executeSelectSimple(dbClient, schema, (q) => q.from("users"));
      ...
      const userCountAfter = await executeSelectSimple(dbClient, schema, (q) => q.from("users"));
  ...
  describe("Complex injection in different contexts", () => {
    it("should handle injection attempts in ORDER BY context", async () => {
      ...
      const users = await executeSelectSimple(dbClient, schema, (q) => q.from("users").take(1), {
        onSql: (result) => {
          usersSql = result;
        },
      });
  ...
  describe("Binary payload injection attempts", () => {
    ...
    it("should handle control characters in parameters", async () => {
      ...
      const results = await executeSelect(
        dbClient,
        schema,
        (q, params) => q.from("products").where((p) => p.name == params.controlCharsPayload),
        { controlCharsPayload },
        {
          onSql: (result) => {
            capturedSql = result;
    ...
    it("should handle SQL injection with special characters", async () => {
      ...
      const results = await executeSelect(
        dbClient,
        schema,
        (q, params) => q.from("users").where((u) => u.name == params.mixedPayload),
        { mixedPayload },
        {
          onSql: (result) => {
            capturedSql = result;
  ...
  describe("Complex nested injection attempts", () => {
    it("should handle nested conditions with injection attempts", async () => {
      ...
      const users = await executeSelectSimple(dbClient, schema, (q) => q.from("users"));
