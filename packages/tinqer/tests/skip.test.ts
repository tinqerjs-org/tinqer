...
describe("SKIP Operations", () => {
  describe("skip()", () => {
    it("should parse skip with constant number", () => {
      const query = (q: QueryBuilder<TestSchema>) => q.from("users").skip(10);
    ...
    it("should parse skip(0)", () => {
      const query = (q: QueryBuilder<TestSchema>) => q.from("users").skip(0);
      const result = parseQuery(query);
    ...
    it("should parse skip before take (pagination pattern)", () => {
      const query = (q: QueryBuilder<TestSchema>) => q.from("users").skip(20).take(10);
      const result = parseQuery(query);
    ...
    it("should parse skip with external parameter", () => {
      const query = (q: QueryBuilder<TestSchema>, p: { offset: number }) =>
        q.from("users").skip(p.offset);
      const result = parseQuery(query);
    ...
    it("should parse skip with arithmetic expression", () => {
      const query = (q: QueryBuilder<TestSchema>, p: { page: number; pageSize: number }) =>
        q.from("users").skip((p.page - 1) * p.pageSize);
      const result = parseQuery(query);
