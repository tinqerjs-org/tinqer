...
describe("WHERE Operation", () => {
  describe("Comparison Operators", () => {
    it("should parse equality comparison (==)", () => {
      const query = (ctx: QueryBuilder<TestSchema>) => ctx.from("users").where((x) => x.id == 1);
    ...
    it("should parse inequality comparison (!=)", () => {
      const query = (ctx: QueryBuilder<TestSchema>) => ctx.from("users").where((x) => x.age != 30);
      const result = parseQuery(query);
    ...
    it("should parse greater than or equal comparison (>=)", () => {
      const query = (ctx: QueryBuilder<TestSchema>) => ctx.from("users").where((x) => x.age >= 21);
      const result = parseQuery(query);
    ...
    it("should parse less than comparison (<)", () => {
      const query = (ctx: QueryBuilder<TestSchema>) => ctx.from("users").where((x) => x.age < 65);
      const result = parseQuery(query);
  ...
  describe("Logical Operators", () => {
    ...
    it("should parse NOT expression (!)", () => {
      const query = (ctx: QueryBuilder<TestSchema>) => ctx.from("users").where((x) => !x.isActive);
      const result = parseQuery(query);
